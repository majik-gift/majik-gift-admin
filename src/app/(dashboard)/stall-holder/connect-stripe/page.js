...
const Page = () => {
  ...
  if (user?.stripeConnectStatus === "approved") {
    router.push(`/${currRole()}`)
  }

  return (
    <>
      <Container>
        <Stack justifyContent="center" alignItems="center" height={"calc(100vh - 64px)"} spacing={2}>
          <Box><Image src={ConnectSvg} alt="StripRequestIllus" style={{ userSelect: "none", WebkitUserDrag: "none" }} /></Box>
          {user?.stripeConnectStatus === "connect_required" ? (
            <Typography variant="h6" textAlign="center">
              In order for you to be available on the site as a {currRole()}, you must attach your Stripe in order to get payouts.
            </Typography>
          ) : (
            <Typography variant="h6" textAlign="center">
          ...
          {user?.stripeConnectStatus === "connect_required" && (
            <UIButton isLoading={loading} endIcon={<ChevronRight />} variant="contained" onClick={connectStripeHandler}>
              Connect to Stripe
            </UIButton>
          )}
        </Stack>
