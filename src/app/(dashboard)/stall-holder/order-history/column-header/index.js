...
const getOrderColumnHistory = ({ handleClientSecret, handleClickOpen, loading, user } = {}) => [
  {
    minWidth: 120,
    field: 'id',
    headerName: 'Order Id',
    ...
    renderCell: (params) => {
      ...
      return (
        <Stack direction="row" gap={1} alignItems="center" height={1}>
          ...
          {params?.row?.status !== 'pending' && !isAdmin && !isAustralian && isUnpaid && <Tooltip
            title={'Pay'}
            onClick={() => handleClientSecret(params?.row)}
          >
            <UIIconButton size="small" fillable>
              <LocalAtmIcon />
            </UIIconButton>
