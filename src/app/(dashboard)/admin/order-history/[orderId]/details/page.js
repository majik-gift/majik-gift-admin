...
const OrderDetailPage = () => {
  ...
  const renderUserDetails = (data) => {
    console.log('datqa', JSON.stringify(data, null, 2))
    return (
      <Grid2 container spacing={2}>
        ...
        <Grid2 size={{ xs: 12, sm: 6, md: 4 }}>
          ...
          <Typography variant="body1">{`${data?.discount_percentage} %` ?? '-'}</Typography>
        </Grid2>
        {/* Price */}
        <Grid2 size={{ xs: 12, sm: 6, md: 4 }}>
          <Typography variant="h6">Discounted Amount</Typography>
          <Typography variant="body1">{`${data?.discount_amount} AUD` ?? '-'}</Typography>
        </Grid2>
        ...
        <Grid2 size={{ xs: 12, sm: 6, md: 4 }}>
          ...
          <Typography variant="body1">{`${data?.tips_percentage} %` ?? '-'}</Typography>
        </Grid2>
        {/* Tip Amount */}
        <Grid2 size={{ xs: 12, sm: 6, md: 4 }}>
        ...
        <Grid2 size={{ xs: 12, sm: 6, md: 4 }}>
          ...
          <Chip
            label={data?.status === 'completed' ? 'Paid' : data?.status}
            sx={{ textTransform: 'capitalize' }}
        ...
        <Grid2 size={{ xs: 12, sm: 6, md: 4 }}>
          ...
          <Typography variant="body1">
            {`${formatNumber(data?.refund_amount)} AUD`}
          </Typography>
        </Grid2>
  ...
  return (
    <Stack gap={1}>
      ...
      {singleOrder?.order_items?.length > 0 && (
