      ...
      <Stack spacing={2} mt="0.8rem" mb="1.2rem" direction="row">
        <Grid2 container spacing={2} width={'100%'}>
          ...
          <Grid2 item size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
            <UIAutocomplete
              name="stallHolderId"
              placeholder="Select Stall Holder"
              fullWidth
              multiple={false}
              control={null}
              url={`users?role=stall_holder`}
              value={extraParams?.stallHolderId}
              onChange={(newValue) => {
                setExtraParams({
                  productId:null,
                  stallHolderId: newValue
                })
              }}
            />
          </Grid2>
          ...
          <Grid2 item size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
            <UIAutocomplete
              ...
              multiple={false}
              control={null}
              url={extraParams?.stallHolderId?`products?userId[]=${extraParams?.stallHolderId?.value}`:`products?`}
