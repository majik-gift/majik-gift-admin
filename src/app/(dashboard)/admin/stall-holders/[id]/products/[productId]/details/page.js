...
const ProductDetailsPage = () => {
  ...
  useEffect(() => {
    // Assuming you have an API call to fetch this data using productId
    dispatch(getSingleProduct({ params: { id: productId } }));
  }, [productId]);
  ...
  const renderUserDetails = (data) => (
    <Grid2 container spacing={2}>
      ...
      <Grid2 size={{ xs: 12, sm: 6 }}>
        ...
        <Typography variant="body1">{data?.quantity || 'N/A'}</Typography>
      </Grid2>
  ...
  return (
    <>
      <UICard heading={'Product Details'} backButton bottomActions={bottomActions()}>
        <ApiLoader loading={loading}>{singleProduct && renderUserDetails(singleProduct)}</ApiLoader>
      </UICard>
      <RejectionDialog
