...
import { getSingleServices, manageServiceStatus } from '@/store/services/services.thunk';
import { Chip, Grid2, Stack, Typography, Box } from '@mui/material';
...
const ProductDetailsPage = () => {
  ...
  const { singleService, loading, error } = useSelector((state) => state.services);
  ...
  useEffect(() => {
    ...
    dispatch(getSingleServices({ params: { id } }));
  }, [id]);
  ...
  const handleUserStatus = async () => {
    try {
      const { payload } = await dispatch(
        manageServiceStatus({ params: { id }, payload: { registrationStatus: 'approved' } })
      );
      if (payload) {
        addToast({
  ...
  const bottomActions = () => (
    <Stack spacing={1} direction="row">
      <UIButton
        isLoading={loading}
        disabled={loading || singleService?.registration_status !== 'pending'}
  ...
  const renderUserDetails = (data) => (
    <Grid2 container spacing={2}>
      ...
      <Grid2 size={{ xs: 12, sm: 6 }}>
        ...
        <Typography variant="body1">{data?.price || '-'} AUD</Typography>
      </Grid2>
      ...
      {/* Created By */}
      <Grid2 size={{ xs: 12, sm: 6 }}>
        <Typography variant="h6">Created For</Typography>
        <Typography variant="body1">
      ...
      {data?.categories?.length && (
        <Grid2 size={{ xs: 12, sm: 6 }}>
          ...
          <Stack gap={1 / 2} direction="row" flexWrap="wrap">
  ...
  return (
    <>
      <UICard heading={'Service Details'} backButton bottomActions={bottomActions()}>
        <ApiLoader loading={loading}>{singleService && renderUserDetails(singleService)}</ApiLoader>
      </UICard>
      <RejectionDialog
