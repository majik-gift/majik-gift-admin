...
//     async (search = '') => {
//       try {
//         const data = await apiGet(`chat/get-all-users-chats?search=${search}`);
...
//       console.log('🚀 ~ fetchChatMessages ~ data:', data?.response?.details);
...
//       setUserChat((prev) =>
//         prev.map((each) =>
//           each.id === activeChatRef.current?.id ? { ...each, unreadCount: 0 } : each
//         )
//       );
//     }
...
//               handleScroll={() =>
//                 fetchChatMessages(true, activeChat.id, messages?.meta?.nextCursor)
//               }
//               activeChat={activeChat}
//               loading={loading}
