...
const CreateProduct = () => {
  ...
  const radioOptions = [
    {
      label: `Standard (${createdFor?.role === 'admin' ? 0 : settingData?.details?.shop_items_standard_fee} %)`,
      value: 'standard',
    },
    {
      label: `Exclusive Package (${createdFor?.role === 'admin' ? 0 : settingData?.details?.shop_items_extras_package_fee} %)`,
    ...
    if (isUpdate && singleProduct) {
      Object.keys(singleProduct).forEach((key) => {
        if (key === 'categories') {
          ...
          const data = {
            label:
              singleProduct?.created_by?.first_name + ' ' + singleProduct?.created_by?.last_name,
            value: singleProduct?.created_by?.id,
          };
  ...
  const createProductHandler = async (dataToSend) => {
    ...
    const payload = {
      ...
      fee_option: dataToSend?.created_for?.role === 'admin' ? 'standard' : dataToSend?.fee_option,
  ...
  const getAllCategories = () => {
    dispatch(categoriesGet({ userData: null, type: 'product_category' }));
  };
  ...
  const getUsers = async () => {
    try {
      ...
      const { data } = await axiosInstance.get(
        'users?roles[]=admin&roles[]=stall_holder&includeAdmin=true&registration_status=approved'
      );
  ...
  const handleFileDelete = (imageIndex) => {
    console.log('🚀 ~ handleFileDelete ~ imageIndex:', imageIndex);
    if (!isUpdate) {
      return true;
    }
          ...
          <Grid2 container spacing={2}>
            <Grid2 size={{ xs: 12, md: 6, lg: 4 }}>
              <UIAutocomplete
                ...
                //   role: e.role,
                // }))}
                url="users?roles[]=admin&roles[]=stall_holder&includeAdmin=true&registration_status=approved"
                label="Created for"
                name="created_for"
