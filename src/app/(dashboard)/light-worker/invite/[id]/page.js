...
const EventDetailsPage = () => {
  ...
  const [hideActions, seHideActions] = useState(false);
  ...
  const handleUserStatus = async (status) => {
    ...
    try {
      ...
      console.log('🚀 ~ handleUserStatus ~ payload:', payload);
      console.log('🚀 ~ handleUserStatus ~ res:', res);
    } catch (error) {
      console.log(error);
    } finally {
      setLoading(false);
  ...
  const renderUserDetails = (data) => (
    <Grid2 container spacing={2}>
      ...
      <Grid2 size={{ xs: 12, sm: 6 }}>
        ...
        <Typography variant="body1">
          {new Date(data?.end_time).toLocaleDateString('en-GB', {
            day: '2-digit',
            month: 'short',
            year: 'numeric',
          }).replaceAll(" " ,'-') || 'N/A'}
